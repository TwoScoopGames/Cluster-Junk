var BoxPool = require("splat-ecs/lib/box-pool");
...
module.exports = function(ecs, game) {
  ...
  window.boxPoolNames = boxPoolNames;
  ...
  ecs.add(function boxGroupCollider() {
    ...
    for (var i = 0; i < boxPoolNames.length; i++) {
      if (boxPoolNames[i] === "terrain") {
        continue;
      }
      boxPools[boxPoolNames[i]].reset();
    }

    var terrainPopulated = boxPools.terrain && boxPools.terrain.ids.length > 0;
    ...
    for (i = 0; i < ids.length; i++) {
      ...
      if (name === "terrain" && terrainPopulated) {
        continue;
      }
      if (!boxPools[name]) {
    ...
    if (boxPools.player) {
      boxPools["player"].collideOther(boxPools["terrain"], handleCollision);
      boxPools["player"].collideOther(boxPools["trash"], handleCollision);
    }
